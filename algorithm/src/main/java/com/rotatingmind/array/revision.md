...
BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        ...
        // First line: n and m (matrix dimensions)
        String[] dimensions = br.readLine().split(" ");
        int n = Integer.parseInt(dimensions[0]);
        int m = Integer.parseInt(dimensions[1]);

        // Next n lines: the matrix elements
        ...
        int[][] rowMat = new int[m][n];
        for (int i = 0; i < m; i++) {
            rowMat[i][0] = matrix[i][0];
            for (int j = 1; j < n; j++) {
                rowMat[i][j] = rowMat[i][j - 1] + matrix[i][j];
            }
        }

        int[][] colMat = new int[m][n];
        for (int i = 0; i < n; i++) {
            colMat[0][i] = rowMat[0][i];
            for (int j = 1; j < m; j++) {
                colMat[j][i] = colMat[j - 1][i] + rowMat[j][i];
            }
        }
        for (int i = 0; i < q; i++) {
...
Given a m x n matrix mat and an integer k, print a matrix answer where each answer[i][j] is the sum of all elements mat[r][c] for:
...
for r in range(n):
    ...
    for c in range(m):
        ...
        #print(r,c,">>>",x1,y1,x2,y2,">>>",sum_matrics(x1,y1,x2,y2))

        

        

        arr_1[r][c] =  sum_matrics(x1,y1,x2,y2)
