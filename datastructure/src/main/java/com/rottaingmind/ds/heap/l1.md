...
we mark deleted ele = NUll which left many black space for that we can pull all element after Null 1 space back which is again o(n)
...
Any array, we can modle this type of array example:

![img_16.png](img_16.png)

This is BT tree, all levels full apart of last level. because without previous index next index not possible. That is complete BT
...
Q. we have array with N size, how to calculate index of last non leaf node
  ...
  -> Parent of last node which would be N/2, because N is size of array, that node would be last non leaf node
